...
function PerfumeForm({ initialData, onSubmit, onCancel }) {
...
export default function PerfumeProducts() {
  ...
  const handleSubmit = async (formData) => {
    if (editingPerfume) {
      await updatePerfume(editingPerfume.id || editingPerfume._id, formData)
    } else {
      await addPerfume(formData)
    }
  ...
  return (
    <div>
            ...
            <div className="p-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="gold-text font-semibold">{perfume.price?.toLocaleString()} IQD</span>
                  {perfume.originalPrice && (
                    <span className="text-gray-500 line-through text-xs">{perfume.originalPrice?.toLocaleString()}</span>
                  )}
                </div>
      ...
      <Modal
        ...
        title={editingPerfume ? 'Edit Perfume' : 'Add New Perfume'}
      >
        <PerfumeForm
          initialData={editingPerfume || defaultFormData}
